...
class Fraction {
    ...
    public:
        ...
        Fraction(double k):frac(k){
            ...
            int k_int = static_cast<int>(k), count = 0, den=1 , num_int= 0;
            double num;
            if (k<0) {
                num = k_int -k;
            }
            ...
            if ( num < 0.01 ){
                cout << "fraction der double variable "<< k << " ist " << k_int << endl ;
            }
            else{
                ...
                // 求最大公约数，简化分数 num_int / den
                int common = __gcd(den, num_int);
                den = den/ common ;
                num_int = num_int / common ;
